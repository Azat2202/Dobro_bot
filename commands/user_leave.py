...
async def user_left_chat(message: types.Message):
    with UsersDatabaseManager() as db_worker:
        ...
        if partner:
            ...
            if parent:
                db_worker.move_child(
                    message.left_chat_member.id, parent, message.chat.id
                )
                main_message = "Все дети были переданы дедушкам и бабушкам!"
            else:
                children = db_worker.get_children(
                    message.left_chat_member.id, message.chat.id
                )
                for child in children:
                    db_worker.remove_child(
                        message.left_chat_member.id, child, message.chat.id
                    )
                main_message = "Все дети оказались в детдоме!("
        db_worker.divorce(message.left_chat_member.id, message.chat.id)
    await message.reply(
        f"Прощайте, {message.left_chat_member.first_name}\n"
        + main_message
        + "\nВы разведены!"
    )
