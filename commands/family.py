...
async def family_repr(message: types.Message):
    with UsersDatabaseManager() as db_worker:
        edges = [
            [User(data[0], data[1]), User(data[2], data[3])]
            for data in db_worker.get_edges(message.chat.id)
        ]
        marriages = [
            [User(data[0], data[1]), User(data[2], data[3])]
            for data in db_worker.get_marriages(message.chat.id)
        ]
        nodes = [item for sublist in marriages for item in sublist] + [
...
def make_graph(nodes, edges, marriages, chat_name, id: str):
    ...
    for n, k in enumerate(marriages):
        ...
        c.node(
            ...
            label=user_1.name,
            _attributes={
                "fillcolor": "#99B2DD" if len(user_1.name) % 2 == 0 else "#E9AFA3"
            },
        )
        c.node(
            str(user_2),
            label=user_2.name,
            _attributes={
                "fillcolor": "#E9AFA3" if len(user_1.name) % 2 == 0 else "#99B2DD"
            },
        )
        c.edge(
            str(user_1),
            str(user_2),
            _attributes={"color": "black:black", "constraint": "false"},
        )
        f.subgraph(c)
    for nod in nodes:
...
if __name__ == "__main__":
    with UsersDatabaseManager() as db_worker:
        edges = [
            [User(data[0], data[1]), User(data[2], data[3])]
            for data in db_worker.get_edges(-1001819892143)
        ]
        marriages = [
            [User(data[0], data[1]), User(data[2], data[3])]
            for data in db_worker.get_marriages(-1001819892143)
        ]
        nodes = [item for sublist in marriages for item in sublist] + [
            item for sublist in edges for item in sublist
        ]
