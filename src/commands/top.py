...
from src.utility import format_name
...
async def spamers_repr(message: types.Message):
    ...
    data = db_worker.get_messages(message)
    ...
    for user in data:
        ...
        if count > 50_000:
            ...
            rank = 'новичок'
        out += f'{num}. {rank} {format_name(data[0], data[1])} - {count} сообщений\n'
        num += 1
...
async def spamers_repr(message: types.Message):
    ...
    db_worker.inc_message(message.from_user.id, message.chat.id, message.from_user.first_name, message.from_user.last_name)
    out = 'Топ кармы: \n'
    data = db_worker.karma_repr(message)
    num = 1
    for user in data:
        karma = int(user[2])
        if karma < -1000:
