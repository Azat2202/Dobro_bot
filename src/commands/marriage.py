...
from src.database.DatabseManager import *
from src.database.exceptions import *
from src.commands.inlineKeyboards import form_inline_kb
from src.main import dp
from src.utility import format_name, beautiful_time_repr
...
async def marriages_repr(message: types.Message):
    db_worker = DatabaseManager()
    db_worker.inc_message(message.from_user.id, message.chat.id, message.from_user.first_name, message.from_user.last_name)
    data = db_worker.marriages_repr(message)
...
async def agreed(call: types.CallbackQuery):
    ...
    if call.from_user.id != user_id:
        ...
        db_worker.close()
        return
    db_worker.del_marriage(call, chat_id, user_id)
